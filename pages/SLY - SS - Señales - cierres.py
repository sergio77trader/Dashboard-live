...
# ─────────────────────────────────────────────
st.set_page_config(layout="wide", page_title="SLY | SIGNAL MONITOR V54")
...
# Memoria Acumulativa persistente en la sesión
...
# ─────────────────────────────────────────────
# MÁQUINA DE ESTADOS (Búsqueda de Señal & PnL)
# ─────────────────────────────────────────────
...
st.title("🛡️ SLY | SIGNAL MONITOR V54")

with st.sidebar:
    st.header("⚙️ Radar Ops")
    # SELECTOR DE TAMAÑO DE LOTE (MÁX 200)
    ...
    batch_idx = st.selectbox(f"Seleccionar Lote (1-{total_lotes}):", range(total_lotes), format_func=lambda x: f"Lote {x+1}")
        ...
        for i, sym in enumerate(subset):
            try:
                ...
                st.session_state["master_results"][sym] = {
                    "Activo": sym, "Última Señal": sig_date.strftime('%Y-%m-%d') if sig_date else "-",
                    "Estado": "VIGENTE 🟢" if vigente else "CERRADA 🔴", "PnL %": round(pnl, 2),
                    "Veredicto": verd, "Precio": round(current_close, 2),
                    "RSI": round(data['rsi_smooth'].iloc[-1], 1),
                    "Régimen": "ALCISTA" if data['ema52'].iloc[-1] > data['ema260'].iloc[-1] else "BAJISTA"
                }
            except: continue
        st.success(f"Lote de {len(subset)} activos procesado.")
...
if st.session_state["master_results"]:
    ...
    st.info(f"Total activos acumulados: {len(df_res)}")
else:
    st.info("👈 Configurá el tamaño de lote y presiona ACTUALIZAR.")
