...
# ─────────────────────────────────────────────
# MÁQUINA DE ESTADOS (Búsqueda de PnL)
# ─────────────────────────────────────────────
...
with st.sidebar:
    ...
    if st.button("🚀 ANALIZAR LOTE SELECCIONADO", type="primary"):
        ...
        for i, sym in enumerate(subset):
            try:
                ...
                # Actualizar el diccionario maestro (acumular)
                st.session_state["master_results"][sym] = {
                    ...
                    "RSI": round(df_rsi := data['rsi_smooth'].iloc[-1], 1),
...
if st.session_state["master_results"]:
    ...
    df_res = df_res.sort_values(by="Estado", ascending=False)
    ...
    st.info(f"Total activos analizados acumulados: {len(df_res)}")
else:
    st.info("👈 Seleccione un lote y comience el análisis. Los resultados se acumularán aquí.")
