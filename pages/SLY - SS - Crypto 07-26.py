...
# ─────────────────────────────────────────────
st.set_page_config(layout="wide", page_title="SLY | CRYPTO 4H MONITOR")
...
def find_last_signal(df, bear_longs):
    ...
    # Veredicto dinámico (Solo si está VIGENTE)
    if is_active:
...
with st.sidebar:
    ...
    if st.session_state["crypto_symbols"]:
        ...
        if st.button("🚀 2. ACTUALIZAR Y ACUMULAR", type="primary", use_container_width=True):
            ex = get_exchange()
            ...
            for i, sym in enumerate(subset):
                try:
                    ...
                    # Descargamos 1000 velas de 4H (~166 días) para estabilidad de EMA 260
                    ...
                    # Cálculo de PnL Real (Solo si vigente)
                    ...
                    st.session_state["crypto_master_results"][sym] = {
                        ...
                        "RSI 4H": f"{data['rsi_smooth'].iloc[-1]:.1f} {'Subiendo' if data['rsi_smooth'].iloc[-1] > data['rsi_smooth'].iloc[-2] else 'Bajando'}",
...
if st.session_state["crypto_master_results"]:
    ...
    st.subheader("📊 EXPOSICIÓN CRIPTO VIGENTE (4H)")
    if not df_vigentes.empty:
        ...
        for idx, row in summary.iterrows():
            with cols[idx % 3]:
                ...
                """, unsafe_allow_html=True)
    else: st.warning("Sin posiciones vigentes en 4H.")
    ...
    def color_cells(val):
        str_v = str(val)
        if "VIGENTE" in str_v or "MANTENER" in str_v or "ALCISTA" in str_v or "Subiendo" in str_v: 
            return 'background-color: #C8E6C9; color: #1B5E20; font-weight: bold;'
        if "CERRADA" in str_v or "CERRAR" in str_v or "BAJISTA" in str_v or "Bajando" in str_v: 
...
else: st.info("Sincronice e inicie el escaneo de lotes en 4H.")
