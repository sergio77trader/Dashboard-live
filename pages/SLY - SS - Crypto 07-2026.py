...
CRYPTO_SECTORS = {
    "LEADER": ["BTC", "ETH"],
    "LAYER 1": ["SOL", "ADA", "DOT", "AVAX", "MATIC", "NEAR", "FTM", "ALGO", "ATOM", "SUI", "APT", "SEI"],
    "AI & DATA": ["RNDR", "FET", "FIL", "THETA", "AGIX", "OCEAN", "WLD", "ARKM"],
...
# ─────────────────────────────────────────────
# MÁQUINA DE ESTADOS DUAL (4H LONG & SHORT)
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# FILTRO DE EXCHANGES (SOLO BINANCE + KUCOIN PERPS)
# ─────────────────────────────────────────────
@st.cache_resource
def get_exchanges():
    return ccxt.kucoinfutures(), ccxt.binance({'options': {'defaultType': 'future'}})

def fetch_synced_perpetuals(min_vol):
    try:
        ku, bi = get_exchanges()
        # 1. Obtener bases de Binance Futures
        b_m = bi.fetch_markets()
        binance_bases = {m['base'].upper() for m in b_m if m['active'] and m['quote'] == 'USDT'}
        # 2. Obtener mercados y volumen de KuCoin Futures
        k_m = ku.fetch_markets()
        tickers = ku.fetch_tickers()
        
        synced = []
        for m in k_m:
            symbol = m['symbol']
            base = m['base'].upper()
            if m['active'] and m['quote'] == 'USDT' and base in binance_bases:
                vol = tickers.get(symbol, {}).get('quoteVolume')
                if vol is None: vol = tickers.get(symbol, {}).get('info', {}).get('volValue', 0)
                if float(vol or 0) >= min_vol and not any(x in symbol for x in ['UP/', 'DOWN/', '3L', '3S']):
...
# INTERFAZ
# ─────────────────────────────────────────────
st.title("🛡️ SLY | MASTER CRIPTO RADAR 4H")

with st.sidebar:
    st.header("⚙️ Radar Ops")
    vol_filter = st.number_input("Volumen Mín 24h (USDT):", value=5000000)
    if st.button("📡 Sincronizar Binance + KuCoin"):
        st.session_state["crypto_list"] = fetch_synced_perpetuals(vol_filter)
        st.rerun()

    if st.session_state["crypto_list"]:
        num_activos = len(st.session_state["crypto_list"])
        st.success(f"Activos Sincronizados: {num_activos}")
        l_size = st.number_input("Por Lote:", 10, 200, 50)
        total_l = (num_activos // l_size) + 1
            ...
            for i, sym in enumerate(subset):
                try:
                    ...
                    st.session_state["master_results_crypto"][sym] = {
                        "Activo": sym.split(":")[0].replace("/USDT", ""), "Sector": get_crypto_sector(sym),
