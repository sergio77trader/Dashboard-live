...
CRYPTO_SECTORS = {
    ...
    "DEFI/L2": ["ARB/USDT", "OP/USDT", "LINK/USDT", "UNI/USDT", "AAVE/USDT"],
    "AI/DEPIN": ["RNDR/USDT", "FET/USDT", "FIL/USDT"],
...
# ─────────────────────────────────────────────
# FILTRO DE EXCHANGES (KUCOIN + BINANCE)
# ─────────────────────────────────────────────
@st.cache_resource
def get_exchanges():
...
def fetch_synced_symbols():
    try:
        ...
        # Intersección: solo monedas que están en AMBOS
        common = k_syms.intersection(b_syms)
        filtered = [s for s in common if not any(x in s for x in ['UP/', 'DOWN/', '3L', '3S'])]
