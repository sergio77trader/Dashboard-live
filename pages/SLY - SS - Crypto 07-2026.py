...
CRYPTO_SECTORS = {
    "LEADERS": ["BTC", "ETH", "SOL"],
    "LAYER 1": ["BNB", "ADA", "DOT", "AVAX", "MATIC", "NEAR", "FTM", "ALGO"],
    "AI & DATA": ["RNDR", "FET", "GRT", "FIL", "NEAR"],
    "DEFI/L2": ["ARB", "OP", "LINK", "UNI", "AAVE", "LDO"],
...
def get_sly_indicators(df):
    try:
        # NORMALIZACIÓN CRÍTICA: Forzar nombres que pandas_ta entiende
        df.columns = [c.capitalize() for c in df.columns]
        if 'Vol' in df.columns: df = df.rename(columns={'Vol': 'Volume'})
        
        # Eliminar filas duplicadas o nulas
        ...
        df['ha_color'] = np.where(ha_close > ha_open, "Verde", "Rojo")
        
        # EMAs de Régimen
        df['ema52'] = ta.ema(df['Close'], length=52)
        df['ema260'] = ta.ema(df['Close'], length=260)
        
        return df.dropna(subset=['ema260', 'hist', 'rsi_smooth'])
...
def find_last_signal(df, bear_longs):
    ...
    # Recorrido de la historia para determinar estado actual
    for i in range(1, len(df)):
        ...
        if not is_active:
            if authorized and ha_flip and macd_accel and rsi_ok:
                is_active, last_entry_date, last_entry_px = True, df.index[i], df['Close'].iloc[i]
        else:
            # SALIDA: HA Rojo + MACD bajando + RSI bajando
...
with st.sidebar:
    ...
    vol_input = st.number_input("Volumen Mín 24h (USDT):", value=5000000)
    
    if st.button("📡 1. SINCRONIZAR MERCADO"):
        st.session_state["crypto_list"] = fetch_symbols(vol_input)
        st.rerun()

    if st.session_state["crypto_list"]:
        total = len(st.session_state["crypto_list"])
        st.success(f"Activos: {total}")
        lote_size = st.number_input("Lote:", 10, 200, 50)
        num_lotes = (total // lote_size) + 1
        batch_idx = st.selectbox(f"Seleccionar Lote:", range(num_lotes), format_func=lambda x: f"Lote {x+1}")
        bear_longs = st.checkbox("Habilitar Bear-Longs", value=True)
        
        if st.button("🚀 2. ANALIZAR Y ACUMULAR"):
            ex = get_exchange()
            ...
            for i, sym in enumerate(subset):
                try:
                    ...
                    st.session_state["master_results_crypto"][sym] = {
                        ...
                        "PnL Real": pnl, "Veredicto": verd, "Precio": round(data['Close'].iloc[-1], 4),
...
if st.session_state["master_results_crypto"]:
    ...
    st.subheader("📋 Matriz Cripto 4H (Acumulativa)")
