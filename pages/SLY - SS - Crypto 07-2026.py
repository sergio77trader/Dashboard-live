...
# ─────────────────────────────────────────────
st.set_page_config(layout="wide", page_title="SLY | MASTER CRIPTO")
...
# ─────────────────────────────────────────────
# FILTRO DE EXCHANGES (RE-INGENIERÍA TOTAL)
# ─────────────────────────────────────────────
def fetch_synced_symbols():
    try:
        # Usamos Spot para la comparación de nombres base por ser más estandarizado
        bi = ccxt.binance()
        ku = ccxt.kucoin()
        
        st.info("Sincronizando bases de datos de Binance y KuCoin...")
        
        # Forzamos la descarga completa (fetch_markets) en lugar de load_markets
        b_m = bi.fetch_markets()
        k_m = ku.fetch_markets()
        
        # 1. Extraer monedas base de Binance que operan contra USDT
        binance_bases = {m['base'].upper() for m in b_m if m['quote'] == 'USDT' and m['active']}
        
        # 2. Filtrar KuCoin buscando esas mismas bases
        synced = []
        for m in k_m:
            base = m['base'].upper()
            if m['active'] and m['quote'] == 'USDT' and base in binance_bases:
                # Filtrar tokens apalancados o basura (UP, DOWN, BEAR, BULL, 3L, 3S)
...
with st.sidebar:
    ...
    if st.session_state["crypto_list"]:
        ...
        total_l = (num_activos // l_size) + (1 if num_activos % l_size > 0 else 0)
        batch_idx = st.selectbox(f"Seleccionar Lote:", range(total_l), format_func=lambda x: f"Lote {x+1}")
        bear_longs = st.checkbox("Habilitar Bear-Longs", value=True)
        
        if st.button("🚀 ACTUALIZAR Y ACUMULAR", type="primary"):
            # Usamos KuCoin para la data OHLCV
            ex = ccxt.kucoin({'enableRateLimit': True})
            subset = st.session_state["crypto_list"][batch_idx*l_size : (batch_idx+1)*lote_size if 'lote_size' in locals() else (batch_idx+1)*l_size]
            prog = st.progress(0)
            ...
            for i, sym in enumerate(subset):
                try:
                    ...
                    time.sleep(0.1) # Rate limit security
                except: continue
            st.rerun()
