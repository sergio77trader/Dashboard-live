...
# ─────────────────────────────────────────────
st.set_page_config(layout="wide", page_title="SLY | CRIPTO MASTER 4H")
...
def get_sly_indicators(df):
    try:
        # NORMALIZACIÓN FORZADA: KuCoin (ccxt) -> Metodología SLY
        ...
        return df.dropna(subset=['ema260', 'hist'])
...
def fetch_synced_symbols(min_vol):
    try:
        ku, bi = get_exchanges()
        # 1. Obtener bases disponibles en Binance USDT (Futures o Spot habilitados)
        b_m = bi.load_markets()
        ...
        for s, m in k_m.items():
            if m['active'] and m['quote'] == 'USDT' and m['linear']:
                ...
                if base in binance_bases:
                    # Filtro de Volumen 24h Robusto
                    vol = tickers.get(s, {}).get('quoteVolume')
                    if vol is None: vol = tickers.get(s, {}).get('info', {}).get('volValue', 0)
                    if float(vol or 0) >= min_vol and not any(x in s for x in ['UP/', 'DOWN/', '3L', '3S']):
                        synced.append(s)
...
# INTERFAZ
# ─────────────────────────────────────────────
st.title("🛡️ SLY | CRIPTO DUAL RADAR 4H")
...
with st.sidebar:
    ...
    if st.button("📡 Sincronizar Binance + KuCoin"):
        st.session_state["crypto_list"] = fetch_synced_symbols(vol_filter)
        st.rerun()

    if st.session_state["crypto_list"]:
        num_activos = len(st.session_state["crypto_list"])
        st.success(f"Activos operables: {num_activos}")
        l_size = st.number_input("Por Lote:", 10, 100, 50)
...
else: st.info("Sincronice y analice lotes para ver resultados.")
