...
# ─────────────────────────────────────────────
st.set_page_config(layout="wide", page_title="SLY | MASTER DUAL PERPS")
...
# ─────────────────────────────────────────────
# FILTRO DE PERPETUOS BINANCE + KUCOIN
# ─────────────────────────────────────────────
@st.cache_resource
def get_exchanges():
...
def fetch_synced_perpetuals():
    try:
        ...
        synced = []
        for m in k_m:
            # Filtro: Solo USDT lineales y que existan en Binance
            if m['active'] and m['quote'] == 'USDT' and m['base'].upper() in binance_bases:
...
# INTERFAZ
# ─────────────────────────────────────────────
st.title("🛡️ SLY | MASTER DUAL PERPETUALS 4H")
...
with st.sidebar:
    ...
    if st.session_state["crypto_list"]:
        ...
        st.success(f"Perpetuos detectados: {num_activos}")
        
        l_size = st.number_input("Acciones por Lote:", 10, 100, 50)
        num_lotes = (num_activos // l_size) + (1 if num_activos % l_size > 0 else 0)
...
if st.session_state["master_results_crypto"]:
    ...
    st.subheader("📊 RESUMEN DE EXPOSICIÓN (VIGENTES EN PERPETUOS)")
    ...
    st.subheader("📋 Matriz de Señales Cripto 4H (Binance Synced)")
