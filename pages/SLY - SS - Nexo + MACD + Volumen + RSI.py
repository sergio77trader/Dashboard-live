...
# ─────────────────────────────────────────────
st.set_page_config(layout="wide", page_title="SLY | OMNI-FILTER & RSI")
...
def analyze_triple_cycle(symbol):
    ...
    for tf, config in MACRO_CONFIG.items():
        try:
            ...
            # --- CÁLCULO DE RSI SEMÁNTICO ---
            if tf in ["1S", "1M"]:
                rsi_series = ta.rsi(df['Close'], length=14)
                if rsi_series is not None and len(rsi_series) > 1:
                    curr_rsi = rsi_series.iloc[-1]
                    prev_rsi = rsi_series.iloc[-2]
                    status = "Subiendo" if curr_rsi > prev_rsi else "Bajando"
                    row[f"{tf} RSI"] = f"{curr_rsi:.1f} {status}"
...
# INTERFAZ Y FILTROS
# ─────────────────────────────────────────────
st.title("🛡️ SLY OMNI-FILTER + RSI SEMÁNTICO")
...
with st.sidebar:
    ...
    sel_batch = st.selectbox("Lote:", range(len(batches)), format_func=lambda x: f"Lote {x+1}")
...
if st.session_state["sniper_results"]:
    ...
    def style_matrix(v):
        v_str = str(v)
        if "LONG" in v_str: return 'background-color: #C8E6C9; color: #1B5E20; font-weight: bold;'
        if "SHORT" in v_str: return 'background-color: #FFCDD2; color: #B71C1C; font-weight: bold;'
    ...
    # Reordenar columnas
    cols = ["Activo", "Precio", "1D Signal", "1S Signal", "1M Signal", "1S RSI", "1M RSI", "1D PnL%", "1S PnL%", "1M PnL%", "Vol 2v(S)%"]
    actual_cols = [c for c in cols if c in df.columns]
    remaining = [c for c in df.columns if c not in actual_cols]
    df = df[actual_cols + remaining]
