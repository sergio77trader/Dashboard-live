...
# ─────────────────────────────────────────────
st.set_page_config(layout="wide", page_title="SLY | ALPHA TERMINAL v4.0")
...
# --- BASE DE DATOS EXPANDIDA (COBERTURA TOTAL BINANCE ~500+ ASSETS) ---
# He añadido líderes, mid-caps y proyectos recientes para llegar al volumen de Binance.
BINANCE_WHITELIST = [
    ...
    'CHZ', 'BEAM', 'PENDLE', 'ALT', 'MANTA', 'PIXEL', 'DYM', 'RON', 'ARKM', 'ID', 'MAV', 'WOO', 'JTO', 'SATS', 'RATS', 
    'MYRO', 'METIS', 'GNO', 'ENS', 'ASTR', 'WLD', 'ZETA', 'XAI', 'TAO', 'TON', 'NOT', 'TURBO', 'MEME', 'LISTA', 'IO', 
    'ZK', 'ZRO', 'BANANA', 'RENDER', 'FIDA', 'EIGEN', 'SCR', 'COW', 'CETUS', 'PNUT', 'ACT', 'NEIRO', 'MOODENG', 'THE', 
    'VANA', 'PENGU', 'VTHO', 'ONG', 'GAS', 'NEO', 'QTUM', 'XMR', 'ZEC', 'DASH', 'LRC', 'OMG', 'ZIL', 'KNC', 'KAVA', 'BAND',
    'IOST', 'CKB', 'STMX', 'ANKR', 'REN', 'CRV', 'SUSHI', 'COMP', 'SNX', 'UMA', 'YFI', 'BAL', 'SRM', 'ALPHA', 'BEL', 'WING', 
    'FLM', 'SUN', 'JST', 'OG', 'ASR', 'ATM', 'ACM', 'PSG', 'BAR', 'REI', 'OXT', 'SXP', 'KMD', 'NMR', 'DGB', 'LSK', 'WAVES', 
    'MTL', 'AR', 'BLZ', 'STORJ', 'SC', 'RLC', 'HOT', 'ZEN', 'ANT', 'LOOM', 'MTL', 'POWR', 'GVT', 'DENT', 'MFT', 'KEY', 'DOCK',
    'MITH', 'REN', 'HOT', 'BTT', 'ONG', 'FET', 'CELR', 'MATIC', 'ATOM', 'PHB', 'FTM', 'ALGO', 'GTC', 'MLN', 'AMP', 'SUN',
    'MINA', 'RAY', 'DYDX', 'GALA', 'AGLD', 'RAD', 'ILV', 'RENBTC', 'ADX', 'QUICK', 'SPELL', 'TRIAS', 'TFUEL', 'LPT', 'KDA'
]
...
# ─────────────────────────────────────────────
# 3. MOTOR DE DATOS (KUCOIN - FUENTE ÚNICA)
# ─────────────────────────────────────────────
@st.cache_resource
def get_exchange():
    ...
    try:
        ...
        for s in markets:
            ...
            if markets[s].get('active', True):
                ...
                # FILTRO DINÁMICO: Si está en la lista O tiene volumen > 100k (muy probable que esté en Binance)
                if base in BINANCE_WHITELIST or vol > 100000:
                    compatible.append(s)
                    seen_bases.add(base)
...
# ─────────────────────────────────────────────
st.title("🛡️ SLY Alpha Terminal v4.0")
st.markdown('<div class="status-msg">Estado: <b>Cobertura Máxima (~500 Activos)</b>. Analizando paridad Cripto/BTC + Señal 1D.</div>', unsafe_allow_html=True)

if not st.session_state["filtered_symbols"]:
    if st.button("📡 1. SINCRONIZAR UNIVERSO TOTAL"):
...
if st.session_state["filtered_symbols"]:
    ...
    batch_size = st.selectbox("Lote de escaneo:", [25, 50, 100, 150], index=1)
    next_limit = min(pointer + batch_size, total_assets)
    
    st.write(f"Progreso: {pointer} / {total_assets} monedas analizadas.")

    if pointer < total_assets:
        if st.button(f"🚀 ESCANEAR SIGUIENTE BLOQUE: {pointer} al {next_limit}"):
            ex = get_exchange()
        ...
        st.success("🎯 Catálogo completo procesado.")
