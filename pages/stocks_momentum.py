...
# --- CONFIGURACIÓN ---
st.set_page_config(layout="wide", page_title="Stocks: HA + MACD Matrix")
...
def analyze_timeframe(df):
    ...
    if ha_flip_green and (curr_hist < 0) and hist_subiendo: signal = "🟢 ENTRADA LONG"
    elif ha_flip_red and (curr_hist > 0) and hist_bajando: signal = "🔴 ENTRADA SHORT"
    else:
    ...
    return {
        "Señal": signal, "Precio": df['Close'].iloc[-1],
        "HA": "Verde" if curr_color == 1 else "Rojo",
        "Hist": curr_hist, "Hist_Dir": "↗️" if hist_subiendo else "↘️"
    }
...
if st.session_state.get('run_stock', False):
    ...
    for i, (label, code) in enumerate(tfs):
        ...
        else: results.append({"Temporalidad": label, "Diagnóstico": "Sin Datos"})
    ...
    st.dataframe(df_res.style.applymap(color_row, subset=['Diagnóstico']),
