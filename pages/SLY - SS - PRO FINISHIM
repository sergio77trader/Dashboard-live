...
def get_sly_indicators(df, vol_acum_window=6):
    """
    ...
    try:
        ...
        # ─────────────────────────────────────
        # AGREGADO — ALERTA 1: Volumen Acumulado "comienza a aumentar"
        # (traducido de tu script "Volumen Acumulado + %": suma de volumen en
        # bloques de N velas, comparado contra el bloque anterior de igual
        # tamaño. "Comienza a aumentar" = el cambio % cruza de <=0 a >0,
        # no que ya esté positivo — así detecta el arranque, no todo el tramo.)
        # ─────────────────────────────────────
        acc_vol_actual  = df['Vol'].rolling(vol_acum_window).sum()
        acc_vol_previo  = acc_vol_actual.shift(vol_acum_window)
        pct_change_vol  = np.where(acc_vol_previo > 0, (acc_vol_actual - acc_vol_previo) / acc_vol_previo * 100, 0.0)
        df['pct_change_vol'] = pct_change_vol
        pct_change_vol_prev = df['pct_change_vol'].shift(1)
        df['vol_acum_subiendo'] = (df['pct_change_vol'] > 0) & (pct_change_vol_prev <= 0)
 
        # ─────────────────────────────────────
        # AGREGADO — ALERTA 2: RSI PRO "en verde"
        # (cualquiera de los 2 estados alcistas ya calculados arriba)
        # ─────────────────────────────────────
        df['rsi_en_verde'] = df['rsi_state'].str.contains("ALCISTA")
 
        # ─────────────────────────────────────
        # AGREGADO — ALERTA 3: MACD "creciendo"
        # Cubre exactamente los 2 casos que pediste: subiendo desde abajo
        # hacia 0 (hist<0 y creciendo) o alejándose hacia arriba (hist>0 y
        # creciendo). Ambos se resumen en una sola condición: hist > hist[t-1].
        # ─────────────────────────────────────
        df['macd_creciendo'] = df['hist'] > df['hist'].shift(1)
...
with st.sidebar:
    ...
    # AGREGADO: ventana para la alerta de "Volumen Acumulado comienza a aumentar"
    # (equivalente al script "Volumen Acumulado + %" — mismos bloques de velas)
    st.subheader("3. Alertas")
    vol_acum_window = st.selectbox(
        "Ventana Volumen Acumulado (velas):", [2, 3, 4, 6, 21, 42], index=3,
        help="Se compara la suma de volumen de este bloque de velas contra el bloque "
             "anterior de igual tamaño. La alerta se dispara cuando ese cambio % pasa "
             "de negativo/neutro a positivo (empieza a subir)."
    )
        ...
        if st.button("🚀 ACTUALIZAR Y ACUMULAR", type="primary"):
            ...
            for i, sym in enumerate(subset):
                try:
                    ...
                    data, motivo = get_sly_indicators(df, vol_acum_window=vol_acum_window)
                    ...
                    # AGREGADO: las 3 alertas que pediste
                    vol_acum_subiendo = bool(data['vol_acum_subiendo'].iloc[-1])
                    rsi_en_verde = bool(data['rsi_en_verde'].iloc[-1])
                    macd_creciendo = bool(data['macd_creciendo'].iloc[-1])
                    vol_acum_txt = "SÍ 📈" if vol_acum_subiendo else "-"
                    macd_creciendo_txt = "SÍ 📈" if macd_creciendo else "-"
 
                    alertas_activas = []
                    if vol_acum_subiendo: alertas_activas.append("VOL↑")
                    if rsi_en_verde: alertas_activas.append("RSI🟢")
                    if macd_creciendo: alertas_activas.append("MACD↑")
                    alertas_txt = " + ".join(alertas_activas) if alertas_activas else "-"
                    ...
                    st.session_state["master_results_crypto"][sym] = {
                        ...
                        "Vol Acum ↑": vol_acum_txt,
                        "MACD Creciendo": macd_creciendo_txt,
                        "Alertas": alertas_txt,
                    ...
                    st.session_state["master_results_crypto"][sym] = {
                        ...
                        "Vol Acum ↑": "-", "MACD Creciendo": "-", "Alertas": "-",
                        "Precio": "-", "RSI": "-", "Régimen": "-",
...
if st.session_state["master_results_crypto"]:
    ...
    cols_order = ["Activo", "Sector", "Alertas", "MACD Fuerza", "Estado", "Veredicto", "PnL Real", "Última Señal",
                  "Temporalidad", "Zona RSI", "RSI PRO", "Vol Z-Score", "Vol (USDT)", "Vol Acum ↑",
                  "MACD Creciendo", "Alpha Strike", "Agotamiento", "RSI", "Precio", "Régimen", "Motivo"]
    ...
    def color_cells(val):
        str_v = str(val)
        # AGREGADO: alertas combinadas — cuantas más condiciones a la vez, más intenso
        if "VOL↑" in str_v and "RSI🟢" in str_v and "MACD↑" in str_v:
            return 'background-color: #2E7D32; color: white; font-weight: bold;'
        if str_v.count("↑") + str_v.count("🟢") >= 2 and str_v != "-":
